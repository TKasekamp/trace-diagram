...
var registerTooltip = function(id, divId, svgAll) {
    ...
    svgAll.select(id).on("mouseenter", function(d) {
        console.log("new loop");
        div.transition().duration(200).style("opacity", 1);
        div.style("pointer-events", "auto").style("left", (d3.event.pageX) + "px").style("top", (d3.event.pageY + 30) + "px");
        onT = true;
        console.log("ont " + onT);
        onE = true;
        console.log("one " + onE);
        ...
        if (onE && !onT) {
            div.transition().duration(500).style("opacity", 0).style("pointer-events", "none");
...
var registerTooltipNoScroll = function(id, divId, svgAll) {
    var div = d3.select(divId).style("opacity", 0).style("pointer-events", "none");
    svgAll.select(id).on("mouseenter", function(d) {
        div.transition().duration(200).style("opacity", 1);
        div.style("left", (d3.event.pageX) + "px").style("top", (d3.event.pageY + 30) + "px");
    }).on("mouseleave", function(d) {
        div.transition().duration(500).style("opacity", 0);
    });
};
var toolTipListener = function(divId) {
    var div = d3.select(divId).style("opacity", 0);
